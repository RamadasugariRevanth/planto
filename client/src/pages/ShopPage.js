...
const ShopPage = () => {
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
      const filters = {
        ...
        priceRange: [priceRange[0], priceRange[1]], // Ensure correct format
  ...
  //     const data = await productService.getAllProducts(filters); // Pass object
  ...
  const styles = {
    gridRoot: {
      rowGap: '10px', // or `rowSpacing` if you're using MUI's system props
    },
  };
      ...
      {loading ? (
        ...
        {products.map((product) => (
          <Grid item key={product.id} xs={12} sm={6} md={4} lg={3} sx={{ display: "flex", justifyContent: "center" }}>
            <Card sx={{ width: "100%", maxWidth: 300, height: "100%", display: "flex", flexDirection: "column", alignItems: "center" }}>
              <CardMedia
                component="img"
                height="200"
                image={product.image_url}
  ...
  ${Number(product.price) ? Number(product.price).toFixed(2) : '0.00'}
              ...
              <CardActions>
                ...
                <Button size="small" startIcon={<ShoppingCart />} onClick={() => handleAddToCart(product)}>Add to Cart</Button>
              </CardActions>
            </Card>
